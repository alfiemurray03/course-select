...
export default function ContactPage() {
  ...
  if (submitted?.ok) {
    return <main>
      <section className="page-hero contact-page-hero"><div className="container"><div className="eyebrow">Enquiry received</div><h1>Thank you for contacting Aptenvo</h1><p>Your enquiry has been recorded in the Aptenvo contact queue. Keep the reference below when replying or contacting us again.</p></div></section>
      <section className="section"><div className="container contact-success-card">
        ...
        <span>Aptenvo reference</span>
        <h2>{submitted.reference}</h2>
        <p>{submitted.message}</p>
        <div className="contact-success-detail"><Clock3 size={20} /><div><strong>Initial support classification: {submitted.priority}</strong><span>Aptenvo will confirm or change the final priority after reviewing severity, impact and any available workaround.</span></div></div>
        <div className="button-row"><Link className="button button-primary" to="/account">Return to My Aptenvo</Link><Link className="button button-secondary" to="/support">View the Help Centre</Link></div>
      </div></section>
    </main>;
  }
  ...
  return <main>
    <section className="page-hero contact-page-hero"><div className="container contact-hero-grid">
      <div><div className="eyebrow">Contact Aptenvo</div><h1>Tell us what you need help with</h1><p>Use the secure form for course questions, larger orders, paid-order support, enrolment, LMS access, technical problems, billing, complaints and privacy enquiries.</p></div>
      <aside><Headphones size={28} /><div><strong>Aptenvo is your first-line contact</strong><span>Do not contact Highfield directly. We will investigate and manage any provider escalation on your behalf.</span></div></aside>
    </div></section>
    ...
    <section className="section contact-introduction-section"><div className="container contact-route-grid">
      ...
      <article><Users size={24} /><h2>Order and enrolment</h2><p>Provide the Aptenvo order reference and learner email when asking about an existing paid order or learner allocation.</p></article>
    ...
    <section className="section section-muted"><div className="container contact-layout">
      <form className="contact-form-card" onSubmit={submit}>
        <div className="contact-form-heading"><MessageSquareText size={28} /><div><span>Secure Aptenvo enquiry form</span><h2>{topicLabel}</h2><p>Fields marked with * are required. Do not provide passwords, full payment-card numbers or unnecessary sensitive information.</p></div></div>
        ...
        <div className="contact-two-fields">
          <label className="contact-field">Aptenvo order reference <span>Optional</span><input type="text" maxLength={80} value={orderReference} onChange={(event) => setOrderReference(event.target.value)} placeholder="For example, APT-…" /></label>
        ...
        <div className="contact-declarations">
          <label><input type="checkbox" checked={adultConfirmed} onChange={(event) => setAdultConfirmed(event.target.checked)} required /><span>I confirm that I am aged 18 or over. Aptenvo is an adult-only service.</span></label>
        ...
        <button className="button button-primary contact-submit-button" type="submit" disabled={submitting}><Send size={18} /> {submitting ? 'Submitting securely…' : 'Submit enquiry to Aptenvo'}</button>
      </form>

      <aside className="contact-sidebar">
        <section className="contact-sidebar-card"><Mail size={28} /><span>Direct email</span><h2>{CONTACT_EMAIL}</h2><p>The secure form is preferred because it creates an Aptenvo reference and records the correct enquiry type. Email remains available when the form cannot be used.</p><a className="button button-secondary full-width" href={`mailto:${CONTACT_EMAIL}`}>Open your email app</a></section>
        <section className="contact-sidebar-card"><Clock3 size={28} /><span>Response framework</span><h2>P1–P4 assessment</h2><dl><div><dt>P1</dt><dd>1 working hour target response</dd></div><div><dt>P2</dt><dd>4 working hours</dd></div><div><dt>P3</dt><dd>8 working hours</dd></div><div><dt>P4</dt><dd>2 working days</dd></div></dl><p>Targets depend on Aptenvo’s reasonable severity assessment. Resolution and back-stop targets are explained in the Help Centre.</p><Link className="contact-sidebar-link" to="/support">Read the support framework</Link></section>
        <section className="contact-sidebar-card warning"><LockKeyhole size={28} /><span>Protect your account</span><h2>Never send secrets</h2><p>Aptenvo will not ask for your Highfield password, Microsoft password, Stripe credentials or full payment-card number. Remove unnecessary personal data before submitting.</p></section>
        <section className="contact-sidebar-card"><ReceiptText size={28} /><span>Existing orders</span><h2>Help us find it quickly</h2><p>Include the Aptenvo order reference, purchaser email, affected learner email and course title. That allows first-line checks before any provider escalation.</p></section>
        <section className="contact-sidebar-card"><ShieldCheck size={28} /><span>Legal operator</span><h2>JA Group Services Ltd</h2><p>Aptenvo is a trading division of JA Group Services Ltd. Company number 16314179. ICO registration ZB877370.</p></section>
      </aside>
    </div></section>
  </main>;
}
