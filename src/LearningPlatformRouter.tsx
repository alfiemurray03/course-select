...
function Plans({ preview = false }: { preview?: boolean }) {
  return <div className="lp-plan-grid">{plans.map((plan) => <article className={plan.featured ? 'featured' : ''} key={plan.name}>{plan.featured && <span className="lp-popular">Most complete individual plan</span>}<small>{plan.audience}</small><h3>{plan.name}</h3><div className="lp-price">{plan.price}<span>/month<br />VAT included</span></div><p>{plan.copy}</p>{!preview && <ul>{plan.features.map((item) => <li key={item}><Check size={16} /> {item}</li>)}</ul>}<Link to={`/lms/sign-in?plan=${encodeURIComponent(plan.name)}`}>{preview ? 'View plan' : `Choose ${plan.name}`} <ArrowRight size={16} /></Link></article>)}</div>;
}

function HomePage() {
  return <main><section className="lp-hero"><div className="lp-container lp-hero-grid"><div><div className="lp-eyebrow"><Sparkles size={16} /> Two ways to learn</div><h1>Unlimited learning and professional training, together in one trusted place.</h1><p>Build practical knowledge through the Sousa Murray Learning Library for one fixed monthly fee, or purchase specialist Highfield Online Training courses individually.</p><div className="lp-actions"><Link className="lp-button light" to="/learning-library/courses">Browse unlimited learning <ArrowRight size={18} /></Link><Link className="lp-button ghost" to="/professional-training">Professional Training</Link></div><div className="lp-assurances"><span><Check size={16} /> Adults aged 18+</span><span><Check size={16} /> VAT-inclusive prices</span><span><Check size={16} /> Sousa Murray eLearning support</span></div></div><aside className="lp-route-panel"><span>Choose your route</span><article><InfinityIcon /><div><h2>Learning Library</h2><p>Unlimited access to included Sousa Murray courses while your plan is active.</p><Link to="/learning-library/courses">Browse the library <ArrowRight size={15} /></Link></div></article><article><GraduationCap /><div><h2>Professional Training</h2><p>Individually purchased Highfield courses delivered through the Highfield LMS.</p><Link to="/courses">Browse professional courses <ArrowRight size={15} /></Link></div></article></aside></div></section>
  <section className="lp-strip"><div className="lp-container"><span><ShieldCheck /> Operated by JA Group Services Ltd</span><span><Headphones /> Sousa Murray first-line support</span><span><LayoutDashboard /> Dedicated Sousa Murray LMS</span><span><Award /> Clear certificate status</span></div></section>
...
function LibraryPage() {
  return <main><section className="lp-page-hero"><div className="lp-container"><div className="lp-eyebrow"><InfinityIcon size={16} /> Sousa Murray Learning Library</div><h1>Unlimited learning for one clear monthly price.</h1><p>Access every Sousa Murray course included in your plan while the subscription remains active. Learn at your pace, complete quizzes, track progress and collect completion certificates.</p><div className="lp-actions"><Link className="lp-button light" to="/learning-library/courses">Browse the course catalogue <ArrowRight /></Link><Link className="lp-button ghost" to="/lms/sign-in">LMS sign in</Link></div></div></section><section className="lp-unlimited-note"><div className="lp-container"><InfinityIcon /><p><strong>“Unlimited” means unlimited access to included library courses for each named learner.</strong> It does not mean unlimited learner accounts, regulated qualifications or included Highfield courses.</p></div></section><section className="lp-section"><div className="lp-container"><header className="lp-section-head"><span>Growing library</span><h2>Practical learning for work, business and everyday progress</h2><p>There are twelve launch courses in the catalogue, with structured lessons, knowledge checks and completion records.</p></header><div className="lp-subject-grid">{subjects.map((subject) => <article key={subject}><BookOpen /><h3>{subject}</h3><p>Clear, self-paced learning with knowledge checks and an appropriate completion record.</p></article>)}</div><div className="lp-centre"><Link className="lp-button primary" to="/learning-library/courses">View all library courses <ArrowRight /></Link></div></div></section><section className="lp-section muted"><div className="lp-container"><header className="lp-section-head"><span>Membership</span><h2>Choose your learner capacity</h2></header><Plans /></div></section></main>;
}
