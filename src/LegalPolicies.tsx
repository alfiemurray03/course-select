...
function PolicyNavigation({ active }: { active: 'terms' | 'privacy' | 'refunds' | 'acceptable-use' }) {
  ...
  return <nav className="policy-suite-nav" aria-label="Aptenvo legal policies">
  ...
  return <main className="policy-page">
    ...
    <section className="section policy-main-section"><div className="container policy-layout">
      ...
      <article className="policy-document">
        <div className="policy-introduction-note"><ShieldCheck size={22} /><p>Aptenvo is a trading division of {COMPANY_NAME}. It is not a separate legal entity. References to “Aptenvo”, “we”, “us” or “our” mean {COMPANY_NAME} unless the context says otherwise.</p></div>
        {children}
        <section className="policy-contact-panel">
          <h2>Contact Aptenvo</h2>
          <p>Questions about this policy should be submitted through the Aptenvo contact page or sent to <a href={`mailto:${CONTACT_EMAIL}`}>{CONTACT_EMAIL}</a>.</p>
          <p><strong>Registered office:</strong> {REGISTERED_OFFICE}</p>
          <Link className="button button-primary" to="/contact">Contact Aptenvo <ArrowRight size={17} /></Link>
        </section>
      </article>
    </div></section>
  </main>;
...
export function TermsOfUsePage() {
  const contents: Array<[string, string]> = [
    ['scope', 'Scope and acceptance'], ['operator', 'Who operates Aptenvo'], ['eligibility', 'Eligibility and 18+ rule'],
    ['roles', 'Aptenvo and Highfield roles'], ['accounts', 'My Aptenvo and JA Group Services ID'], ['orders', 'Orders and contract formation'],
  ...
  return <PolicyPage
    active="terms"
    eyebrow="Legal terms"
    title="Aptenvo Terms of Use"
    summary="These terms govern use of the Aptenvo website, My Aptenvo, JA Group Services ID, ordering, learner enrolment and the online-training services supplied through Aptenvo."
    icon={<Scale size={29} />}
    contents={contents}
  >
    <PolicySection id="scope" title="1. Scope and acceptance">
      <p>These Terms of Use apply when you browse Aptenvo, create or use a customer account, save information, submit an enquiry, place an order, provide learner details, upload a learner list, or use any related Aptenvo feature.</p>
      <p>By using Aptenvo, you agree to these terms, the <Link to="/privacy">Privacy Policy</Link>, the <Link to="/refunds">Refunds Policy</Link> and the <Link to="/acceptable-use">Acceptable Use Policy</Link>. Additional written terms may apply to a direct organisation order, quotation or separately agreed service. Where additional signed terms conflict with these terms, the signed terms take priority for that order.</p>
      <p>Nothing in these terms removes rights that cannot lawfully be excluded, including mandatory consumer rights.</p>
    </PolicySection>

    <PolicySection id="operator" title="2. Who operates Aptenvo">
      <p>Aptenvo is operated by {COMPANY_NAME}, a private limited company registered in England and Wales under company number 16314179. Our registered office is {REGISTERED_OFFICE}. Our ICO registration number is ZB877370.</p>
      <p>{COMPANY_NAME} is the seller, account provider, contractual customer contact and first-line support provider for Aptenvo purchases.</p>
    </PolicySection>

    <PolicySection id="eligibility" title="3. Eligibility and the 18+ rule">
      <p>Aptenvo is an adult-only service. You must be at least 18 years old to browse beyond the age gate, create an account, make a purchase, submit a contact request or act as an authorised organisation contact.</p>
    ...
    <PolicySection id="roles" title="4. The roles of Aptenvo and Highfield">
      <p>{COMPANY_NAME}, trading through Aptenvo, is an authorised reseller of Highfield Online Training. Customers purchase from Aptenvo and remain customers of Aptenvo.</p>
      <div className="policy-role-grid">
        <article><Building2 size={23} /><h3>Aptenvo</h3><p>Markets the available courses, provides course information, operates the website and customer account, accepts payment, records orders, validates learner details, arranges enrolment, provides first-line support and manages complaints or provider escalation.</p></article>
        <article><BookOpen size={23} /><h3>Highfield Online Training</h3><p>Provides the course content and operates the Learning Management System used by enrolled learners. After Aptenvo completes enrolment, Highfield sends the learner access instructions.</p></article>
      </div>
      <p>Using Highfield’s Learning Management System does not transfer the Aptenvo customer relationship to Highfield. Customers should contact Aptenvo first. Where provider assistance is required, Aptenvo will manage the escalation.</p>
    </PolicySection>

    <PolicySection id="accounts" title="5. My Aptenvo and JA Group Services ID">
      <p>My Aptenvo allows customers to save customer information, adult learner details and reusable baskets. Signed-in accounts are authenticated through <strong>JA Group Services ID</strong>, the customer identity service operated by {COMPANY_NAME} using Microsoft Entra External ID.</p>
      <PolicyList items={[
      ...
      <p>JA Group Services ID may offer sign-in methods configured in the relevant customer user flow, such as email credentials, one-time passcodes or supported third-party identity providers. Aptenvo does not receive or store the password used with an external identity provider.</p>
    ...
    <PolicySection id="orders" title="6. Orders and contract formation">
      ...
      <p>By submitting an order, you offer to purchase the selected licence or licences using the information supplied. Stripe payment authorisation does not by itself require us to accept an order. A contract is formed when Aptenvo sends an order acceptance or begins enrolment, whichever happens first.</p>
    ...
    <PolicySection id="pricing" title="7. Pricing and payment">
      <p>Customer-facing prices are shown in pounds sterling and include VAT where stated. Quantity pricing is calculated separately for each course. The live Aptenvo catalogue and checkout service verify the applicable price before Stripe Checkout is created.</p>
      <p>Payments are processed securely through Stripe. Aptenvo does not store full payment-card numbers or card security codes. Stripe’s own terms and privacy information also apply to its payment processing.</p>
    ...
    <PolicySection id="licences" title="8. Course licences and larger orders">
      ...
      <p>The public Aptenvo checkout accepts no more than 25 learner-course licences in a basket. A requirement for 26 licences or more must be arranged directly with Aptenvo. Customers must not divide a larger requirement into several smaller online orders to avoid the direct-order process.</p>
    ...
    <PolicySection id="learners" title="9. Customer and learner information">
      ...
      <PolicyList items={[
        ...
        <>You must tell learners that Aptenvo and Highfield will use their information for enrolment, course delivery and support.</>,
        ...
        <>You are responsible for losses or delays caused by inaccurate, incomplete or unauthorised learner information, except where Aptenvo caused the error.</>,
      ]} />
    </PolicySection>

    <PolicySection id="delivery" title="10. Enrolment, activation and delivery">
      <p>After successful payment, the order enters Aptenvo’s enrolment workflow. Aptenvo checks the order and learner allocation, then provides the information needed for Highfield to create course access.</p>
      <p>Highfield normally emails the enrolled learner with LMS access instructions. The Aptenvo payment or order confirmation is not the same as the LMS access email.</p>
    ...
    <PolicySection id="course-use" title="11. Use of course content and certificates">
      ...
      <p>The course page and qualification notice should be read before purchase. Aptenvo does not make employment, regulatory or professional decisions for customers.</p>
    </PolicySection>
    ...
    <PolicySection id="technical" title="12. Technical requirements">
      ...
      <p>Customers should use the Help Centre troubleshooting steps before reporting a technical problem and should provide the course title, learner email, device, browser, affected page and steps already tried. Passwords must never be sent to Aptenvo.</p>
    </PolicySection>

    <PolicySection id="support" title="13. Support and service targets">
      <p>Aptenvo is the first-line support contact for orders, enrolment, LMS access, account issues and course problems. Customers should not contact Highfield directly unless Aptenvo expressly instructs them to do so.</p>
      <p>Aptenvo uses a P1–P4 framework to prioritise support based on severity, number of affected users, business impact and available workarounds. Initial automated or customer-selected priority is provisional and may be changed after staff review.</p>
    ...
    <PolicySection id="intellectual-property" title="16. Intellectual property">
      <p>The Aptenvo name, website design, software, databases, written content and branding are owned by or licensed to {COMPANY_NAME}. Highfield names, course materials and related marks belong to their respective owners.</p>
    ...
    <PolicySection id="availability" title="17. Availability and changes to the service">
      <p>We may maintain, update, improve, suspend or withdraw parts of Aptenvo. Course titles, descriptions, prices, provider availability, technical requirements and catalogue content may change before an order is accepted.</p>
      ...
      <p>Where a material change affects a paid and unfulfilled order, Aptenvo will provide an appropriate remedy, replacement option or refund where required.</p>
    </PolicySection>
    ...
    <PolicySection id="liability" title="18. Liability">
      ...
      <p>For consumers, Aptenvo is responsible for foreseeable loss caused by our breach or failure to use reasonable care and skill. We are not responsible for loss that was not foreseeable, business loss arising from consumer use, or loss caused by inaccurate information, misuse, unsupported equipment, unauthorised access or failure to follow reasonable instructions.</p>
      <p>For business customers, Aptenvo is not responsible for indirect or consequential loss, loss of profit, revenue, opportunity, goodwill or anticipated savings, except where the law prohibits that exclusion. Any separate business quotation or direct-order agreement may set additional liability terms.</p>
      <p>Aptenvo does not guarantee that completing a course will satisfy a specific employer, regulator, insurer or professional body unless that outcome is expressly stated in writing.</p>
    </PolicySection>
    ...
    <PolicySection id="communications" title="20. Electronic communications">
      <p>You agree that Aptenvo may provide order confirmations, account messages, security notices, policy updates, support communications and enrolment information electronically using the email address or account provided.</p>
    ...
    <PolicySection id="complaints" title="21. Complaints">
      <p>Complaints should be submitted to Aptenvo through the <Link to="/contact?topic=complaint">contact form</Link>. Include the order reference, relevant learner email, dates, issue and the outcome sought.</p>
      <p>Aptenvo remains responsible for managing the customer complaint even where provider information is required. Data-protection complaints may also be made to the Information Commissioner’s Office, and consumer advice may be available through the appropriate UK consumer-advice service.</p>
    </PolicySection>
...
export function PrivacyPolicyPage() {
  ...
  return <PolicyPage
    ...
    title="Aptenvo Privacy Policy"
    summary="This policy explains how JA Group Services Ltd collects, uses, stores and shares personal information through Aptenvo, My Aptenvo, JA Group Services ID, checkout, enrolment and support."
    icon={<Fingerprint size={29} />}
    contents={contents}
  >
    <PolicySection id="controller" title="1. Controller and scope">
      <p>{COMPANY_NAME} is the controller for personal information used to operate Aptenvo, decide how customer accounts and orders are managed, arrange enrolment and provide support.</p>
    ...
    <PolicySection id="adult-service" title="2. Adult-only service">
      <p>Aptenvo is intended only for adults aged 18 or over. We do not knowingly sell courses to, create customer accounts for or enrol anyone under 18.</p>
    ...
    <PolicySection id="information" title="3. Information we collect">
      <div className="policy-data-grid">
        ...
        <article><FileCheck2 size={22} /><h3>Account and saved data</h3><p>My Aptenvo profile, saved learners, named saved baskets, account preferences and locally stored device data.</p></article>
      ...
      <p>Do not provide passwords, full payment-card details, criminal-offence information, health information or other special-category data unless it is strictly necessary and Aptenvo has specifically asked for it through an appropriate secure route.</p>
    </PolicySection>
    ...
    <PolicySection id="purposes" title="5. Purposes and lawful bases">
      <div className="policy-table-wrap"><table className="policy-table"><thead><tr><th>Purpose</th><th>Typical information</th><th>Lawful basis</th></tr></thead><tbody>
        <tr><td>Provide My Aptenvo, process orders, take payment, enrol learners and deliver support</td><td>Identity, contact, account, order, learner and payment-reference data</td><td>Necessary for a contract or steps requested before a contract</td></tr>
        ...
        <tr><td>Prevent fraud, misuse, account compromise and security incidents</td><td>IP, device, sign-in, session, transaction and audit data</td><td>Legitimate interests in protecting customers, Aptenvo and service providers; legal obligation where applicable</td></tr>
    ...
    <PolicySection id="identity" title="6. JA Group Services ID and Microsoft Entra External ID">
      <p>JA Group Services ID is the customer identity and access service used for My Aptenvo. It is operated by {COMPANY_NAME} using a dedicated Microsoft Entra External ID customer tenant.</p>
      <PolicyList items={[
        ...
        <>Aptenvo receives identity-token claims needed to recognise the account, such as a subject identifier, email, display name, given name and family name.</>,
        <>Aptenvo converts the identity subject into an internal account identifier and uses a signed, secure session cookie to keep the customer signed in.</>,
        <>A normal Aptenvo session lasts up to seven days unless the user signs out, the secret changes, the account is restricted or the session is otherwise invalidated.</>,
        <>Temporary sign-in transaction data is used to protect the authorisation flow and normally expires within minutes.</>,
        <>Aptenvo does not receive the password used with Microsoft, Google or another configured external identity provider.</>,
      ]} />
    ...
    <PolicySection id="device-storage" title="7. Device storage, cookies and similar technologies">
      <p>Aptenvo uses strictly necessary cookies and browser storage to operate security, sign-in, age confirmation, baskets, preferences and checkout.</p>
      <div className="policy-table-wrap"><table className="policy-table"><thead><tr><th>Item</th><th>Purpose</th><th>Typical duration</th></tr></thead><tbody>
        <tr><td><code>aptenvo_session</code></td><td>Signed JA Group Services ID session for My Aptenvo</td><td>Up to 7 days</td></tr>
      ...
      <p>Device-mode data stays in the browser until it is submitted through checkout, synchronised after sign-in or otherwise sent to Aptenvo. Clearing browser data may permanently remove device-only baskets or saved details.</p>
    </PolicySection>

    <PolicySection id="payments" title="8. Payments and Stripe">
      <p>Stripe processes payment-card and transaction information when a customer enters Stripe Checkout. Aptenvo receives limited transaction details such as the Stripe session or payment reference, amount, currency, status, customer email, refund status and fraud or failure information.</p>
    ...
    <PolicySection id="enrolment" title="9. Learner enrolment and Highfield">
      <p>Aptenvo shares the learner’s legal name, enrolment email, selected course and related fulfilment information with Highfield where necessary to create and operate LMS access, deliver the course, provide completion evidence and resolve provider-side issues.</p>
      ...
      <p>The customer relationship, order, payment, first-line support and complaint route remain with Aptenvo. This does not prevent Highfield from having its own legal responsibilities for personal information processed in its systems.</p>
    </PolicySection>
    ...
    <PolicySection id="uploads" title="10. Uploaded learner documents">
      ...
      <p>Aptenvo may reject, quarantine or delete files that are unsafe, corrupted, unsupported, excessive, unrelated or inconsistent with the order.</p>
    </PolicySection>

    <PolicySection id="support" title="11. Support, contact requests and priority assessment">
      <p>The Aptenvo contact form records the enquiry type, reported impact, message, customer information, optional order and learner references, technical context and a unique Aptenvo reference.</p>
      <p>The system assigns an <strong>initial</strong> P1–P4 priority from the impact selected by the person submitting the form. This is a triage aid only. Aptenvo staff review the circumstances and may confirm or change the priority. The initial classification does not make a solely automated decision with legal or similarly significant effect.</p>
    ...
    <PolicySection id="rights" title="16. Your data-protection rights">
      ...
      <p>Rights are not absolute and exemptions may apply. We may request information to verify identity and authority. Submit a request through <Link to="/contact?topic=data-protection">Contact Aptenvo</Link>.</p>
    ...
    <PolicySection id="marketing" title="17. Marketing">
      <p>Aptenvo will only send electronic marketing where there is an appropriate lawful basis. Marketing messages will identify the sender and provide a clear unsubscribe method.</p>
    ...
    <PolicySection id="complaints" title="18. Privacy complaints">
      <p>Please contact Aptenvo first so we can investigate. Select the data-protection enquiry type and explain the concern.</p>
      <p>You also have the right to complain to the Information Commissioner’s Office. Current contact and complaint information is available from the ICO website. This right is not affected by contacting Aptenvo first.</p>
    </PolicySection>
...
export function RefundsPolicyPage() {
  const contents: Array<[string, string]> = [
    ...
    ['provider-cancellation', 'Provider or Aptenvo cancellation'], ['request', 'How to request a refund'], ['processing', 'How refunds are processed'],
  ...
  return <PolicyPage
    ...
    title="Aptenvo Refunds and Cancellation Policy"
  ...
  >
    <PolicySection id="scope" title="1. Scope">
      <p>This policy applies to course licences purchased from {COMPANY_NAME} through Aptenvo. It forms part of the Aptenvo Terms of Use.</p>
    ...
    <PolicySection id="consumer-right" title="2. Consumer cancellation period before supply">
      ...
      <p>To cancel before activation, contact Aptenvo immediately using the billing and refund enquiry type. Include the Aptenvo order reference, purchaser email, learner email and course title.</p>
      <p>A cancellation request is not effective merely because the learner does not open an email. Aptenvo must receive the request through a contact route we can reasonably verify.</p>
    </PolicySection>

    <PolicySection id="immediate-supply" title="3. Request for immediate digital supply">
      <p>Aptenvo normally begins enrolment promptly after successful payment. Before checkout, the purchaser is asked to:</p>
      <PolicyList items={[
        <>expressly request Aptenvo to begin enrolment and digital supply without waiting for the 14-day cancellation period; and</>,
      ...
      <p>Aptenvo records that declaration with the checkout and order audit trail. If the required express request and acknowledgement were not obtained, the consumer’s legal cancellation position may be different.</p>
    </PolicySection>

    <PolicySection id="activation" title="4. Once access is activated or learning begins">
      <p>Once course access has been activated or the learner has begun the course, Aptenvo does not provide a change-of-mind refund where immediate supply was expressly requested and the loss of the cancellation right was acknowledged.</p>
    ...
    <PolicySection id="business" title="5. Business and organisation customers">
      <p>Business customers do not normally have the statutory 14-day consumer cooling-off period. An online business order may be cancelled only where Aptenvo agrees, the course has not been activated and no non-recoverable fulfilment cost has been incurred.</p>
      ...
      <p>Nothing in this section removes rights a business customer may have where Aptenvo breaches the contract or the supplied service does not conform to agreed terms.</p>
    </PolicySection>
    ...
    <PolicySection id="available" title="6. When a refund, repair, replacement or other remedy may be available">
      <PolicyList items={[
        ...
        <>Aptenvo accepts payment but cannot supply the purchased course or a suitable agreed replacement.</>,
        <>Aptenvo or Highfield cancels the course before fulfilment.</>,
        ...
        <>The wrong course or learner was activated because of an error caused by Aptenvo.</>,
    ...
    <PolicySection id="not-available" title="7. When a change-of-mind refund is not available">
      <PolicyList items={[
        ...
        <>an employer, regulator or third party refuses to accept a course where Aptenvo did not expressly guarantee acceptance;</>,
    ...
    <PolicySection id="details" title="8. Incorrect learner or enrolment details">
      <p>Contact Aptenvo immediately if a legal name or enrolment email is wrong. Before activation, we will normally correct valid details without charge where reasonably possible.</p>
      ...
      <p>Aptenvo may ask for evidence of identity, authority and the correct information before changing an enrolled learner.</p>
    </PolicySection>

    <PolicySection id="technical" title="9. Technical or LMS problems">
      <p>A technical problem does not automatically entitle the customer to an immediate refund. Aptenvo must be given a reasonable opportunity to investigate and provide troubleshooting, restore access, correct an enrolment or obtain Highfield assistance.</p>
      ...
      <p>Where the content or service does not conform and cannot be repaired, replaced or repeated within a reasonable time and without significant inconvenience, Aptenvo will provide the remedy required by law.</p>
    </PolicySection>

    <PolicySection id="provider-cancellation" title="10. Cancellation by Aptenvo or the provider">
      <p>If Aptenvo cannot fulfil a paid order because a course is withdrawn, unavailable or materially changed before activation, we may offer an equivalent replacement, account credit where agreed, or a refund for the affected unfulfilled licence.</p>
    ...
    <PolicySection id="request" title="11. How to request cancellation or a refund">
      <p>Use <Link to="/contact?topic=billing-refund">Contact Aptenvo</Link> and select the payment, billing or refund enquiry type. Provide:</p>
      <PolicyList items={[
        <>the Aptenvo order reference;</>,
      ...
      <p>Aptenvo may request further information to verify the customer, transaction, learner and entitlement.</p>
    </PolicySection>
    ...
    <PolicySection id="chargebacks" title="13. Chargebacks and payment disputes">
      <p>Customers should contact Aptenvo before raising a chargeback so we can investigate and provide a remedy where appropriate. A chargeback does not itself cancel an activated licence or resolve a contractual dispute.</p>
    ...
    <PolicySection id="rights" title="14. Statutory rights">
      ...
      <p>Consumers may obtain independent advice from the appropriate UK consumer-advice service. Complaints can also be submitted through Aptenvo’s complaints route.</p>
    </PolicySection>
  </PolicyPage>;
}
...
export function AcceptableUsePolicyPage() {
  ...
  return <PolicyPage
    ...
    title="Aptenvo Acceptable Use Policy"
    summary="This policy sets the rules for using the Aptenvo website, My Aptenvo, JA Group Services ID, baskets, learner uploads, contact channels and purchased course licences."
    icon={<LockKeyhole size={29} />}
    contents={contents}
  >
    <PolicySection id="scope" title="1. Scope">
      <p>This policy applies to everyone who accesses Aptenvo, including visitors, account holders, purchasers, organisation contacts, learners and anyone acting on their behalf.</p>
      <p>It applies to the website, APIs, My Aptenvo, JA Group Services ID sessions, baskets, uploads, contact and support forms, order processing and use of course access obtained through Aptenvo.</p>
    </PolicySection>

    <PolicySection id="general" title="2. General standard of use">
      <p>You must use Aptenvo lawfully, honestly, safely and only for its intended purpose of obtaining and administering adult online training.</p>
    ...
    <PolicySection id="uploads" title="6. Learner-list uploads and personal information">
      <p>Uploads must be necessary for the relevant Aptenvo order and must contain only accurate information you are authorised to provide.</p>
      <PolicyList items={[
        <>Do not upload malware, scripts, executable content, password-protected material that Aptenvo cannot review, or files designed to exploit a parser or system.</>,
        <>Do not upload passwords, payment-card details, bank credentials, identity documents, criminal records, health data or other sensitive information unless Aptenvo has specifically requested it through an approved secure process.</>,
    ...
    <PolicySection id="technical" title="7. Prohibited technical conduct">
      <PolicyList items={[
        ...
        <>using Aptenvo infrastructure to host, relay or distribute unrelated content or communications.</>,
      ]} />
    </PolicySection>
    ...
    <PolicySection id="orders" title="8. Orders, payments and commercial misuse">
      <PolicyList items={[
        ...
        <>Do not resell Aptenvo courses or represent yourself as an authorised reseller, agent or partner without written authorisation.</>,
      ]} />
    </PolicySection>

    <PolicySection id="communications" title="9. Contact, complaints and support conduct">
      <p>Aptenvo welcomes genuine questions, complaints and accessibility requests. Communications must remain lawful and relevant.</p>
      <PolicyList items={[
        ...
        <>Do not publish private support correspondence dishonestly or out of context to impersonate Aptenvo or Highfield.</>,
      ]} />
      <p>Critical or persistent abusive communications may be restricted while Aptenvo continues to meet any non-excludable legal obligations.</p>
    </PolicySection>

    <PolicySection id="intellectual-property" title="10. Intellectual property and branding">
      <p>You may link to public Aptenvo pages in a fair and lawful way. You must not frame the website, copy substantial parts of the catalogue, use Aptenvo or Highfield branding to suggest endorsement, or register confusingly similar domains, accounts or business names.</p>
    ...
    <PolicySection id="security" title="11. Reporting security concerns">
      <p>Do not exploit a suspected vulnerability. Stop testing, preserve evidence and contact Aptenvo with enough information for us to investigate safely.</p>
    ...
    <PolicySection id="reporting" title="13. Reporting misuse or appealing action">
      <p>Report suspected misuse through <Link to="/contact">Contact Aptenvo</Link>. Include relevant references, dates and evidence without exposing passwords or unrelated personal information.</p>
      <p>A person affected by an account or order restriction may contact Aptenvo to request a review. We may require identity and authority checks before discussing the account.</p>
    </PolicySection>
  </PolicyPage>;
}
